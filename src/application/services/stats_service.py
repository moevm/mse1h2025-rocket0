...
class StatsService:
        ...
        try:
            ...
            if response.status_code != HTTPStatus.OK:
                general_logger.warning(
                    f"Failed to get info for user {user_id}: Status {response.status_code}")
                return None

            user_info: dict[str, Any] = response.json()
            if user_info.get("success") is False:
                general_logger.warning(
                    f"API indicated failure for user {user_id}: {user_info.get('error', 'No error message')}")
                return None

            return user_info.get("user", {}).get("roles", [])
        except Exception as e:
            general_logger.error(
                f"Exception fetching roles for user {user_id}: {e}", exc_info=True)
            return None
    ...
    async def _fetch_user_roles(self, bot: Bot, user_ids: list[str]) -> dict[str, list[str]]:
        ...
        tasks = [_fetch_single_user_roles_async_wrapper(
    ...
    async def collect_stats(
        ...
        if all_channels:
            target_channels = [
                ch for ch in raw_channels if ch.get("name") in all_channels]
            channel_names = {c["_id"]: c.get(
                "name", "Unknown") for c in target_channels}
        ...
        for channel in target_channels:
            ...
            history = bot.get_group_history(
                channel_id, oldest=from_date, latest=to_date)
        ...
        if all_users:
            user_ids_by_name = {uid for uid,
                                name in user_names.items() if name in all_users}
            target_user_ids = user_ids_by_name

        if all_roles:
            role_list_lower = {r.lower() for r in all_roles}
            user_ids_by_role = {uid for uid, roles in user_roles.items() if any(
                role.lower() in role_list_lower for role in roles)}
        ...
        if target_user_ids is not None:
            final_users = {uid: stats for uid,
                           stats in users.items() if uid in target_user_ids}
            final_user_names = {
                uid: name for uid, name in user_names.items() if uid in target_user_ids}
